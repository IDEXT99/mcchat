...
function fmtLastSeen(ts) {
  ...
  // If it's today in IST, just show the time.
...
function enterApp(userId, userData) {
  ...
  // ── Presence: /users/{username}/presence = { isOnline, lastSeen } ──
  // Setting this on login, and queuing an onDisconnect hook, means the
  // database always reflects the truth even if the tab is closed abruptly
  // (network drop, crash, closing the laptop) rather than a clean logout.
...
// ══════════════════════════════════════════════════════════════
//  PRESENCE — live "Online" / "Last seen" in the DM header
// ══════════════════════════════════════════════════════════════
function listenPresence(otherUserId) {
  ...
  const fn = snap => {
    ...
    if (!p || p.isOnline) {
      subtitle.innerHTML = `<span class="presence-dot"></span> <span style="color:#00F2FE;">Online</span>`;
    } else {
...
function listenMessages() {
  ...
  const fn = snap => {
    const data = snap.val() || {};
    // Firebase push IDs are lexicographically chronological by design, so
    // sorting by key is reliable even in the instant before a serverTimestamp()
    // placeholder resolves to a real number. We still keep ts for display.
...
function renderMessages(msgs) {
  ...
  // Messages are already sorted oldest → newest, so simply appending them
  // in order places the newest message at the END (bottom) of the container.
  msgs.forEach(msg => area.appendChild(renderMessageRow(msg)));

  // Always force the view down to the newest message after any render.
  scrollBottom();
}
...
function renderMessageRow(msg) {
  ...
  // For audio messages, don't cap width with max-w-[78%] — that percentage
  // is relative to the (often narrow) mobile viewport and is exactly what
  // crushes the player. Instead let the column size to its content, and
  // let the audio wrapper's own min-width be the thing that protects it.
  const col = fc("div", {
    class: `flex flex-col gap-1 ${isAudio ? "shrink-0" : "max-w-[78%]"} ${mine ? "items-end" : "items-start"}`
  });
  ...
  // bubble — audio gets a transparent, minimally-padded wrapper so the
  // native player isn't squished by the standard text-bubble padding.
  // Media bubbles also use a standard rounded-2xl radius (never a pill
  // shape) so a wide rectangular audio player sits naturally inside.
  const bubble = fc("div", {
    class: `relative rounded-2xl ${isAudio ? "audio-bubble" : "px-3.5 py-2.5"} ${mine ? "bubble-out" : "bubble-in"}`,
...
function renderMessageContent(msg) {
  ...
  if (msg.type === "audio") {
    // The audio element is wrapped in its own fixed-width container with
    // flex-shrink:0, so the surrounding flex chain (.msg-row > .col > bubble)
    // can never crush it to 0px on narrow mobile viewports — only the
    // wrapper's own declared width governs its size.
    const wrap = fc("div", { class: "w-[250px] sm:w-[300px] min-w-[200px] max-w-full flex-shrink-0" });
    const audio = fc("audio", {
      controls: "true",
      src: msg.media,
      preload: "metadata",
      class: "w-full h-10 outline-none flex-shrink-0",
    });
...
EMOJI_GRID.forEach(e => {
  ...
  // Use mousedown (fires before the input would blur) and preventDefault
  // so focus never leaves msgInput, the cursor position is preserved, and
  // no accidental form submission / Enter-key send can be triggered.
  btn.addEventListener("mousedown", evt => {
    ...
    // keep the auto-grow height in sync
    input.style.height = "auto";
